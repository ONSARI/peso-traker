...
export const MeasurementHistory: React.FC<MeasurementHistoryProps> = ({ entries, onDeleteEntry, measurementUnit }) => {
    ...
    const displayValue = (cmValue: number | null | undefined) => {
    ...
    return (
        <div className="bg-card dark:bg-gray-800 p-4 sm:p-6 rounded-lg shadow-lg">
            <h2 className="text-xl font-bold text-text-primary dark:text-gray-100 mb-4">{t('measurementHistory.title')}</h2>
                        ...
                        {reversedEntries.map(entry => (
                            <tr key={entry.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                <td className="p-3 whitespace-nowrap text-text-primary dark:text-gray-200 font-medium">
                                    {new Date(`${entry.date}T00:00:00`).toLocaleDateString(i18n.language, { year: 'numeric', month: 'long', day: 'numeric' })}
                                </td>
